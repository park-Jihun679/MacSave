<template>
  <div>
    <div class="type-toggle">
      <button :class="['toggle-btn', type === '수입' ? 'income' : '']" disabled>
        수입
      </button>
      <button
    ...
    <div class="modal-content">
      ...
      <div class="form-actions">
        <button @click="updateTransaction" :disabled="!isValid">저장</button>
...
async function updateTransaction() {
  ...
  const endpoint =
    props.type === '수입' ? `/incomes/${updated.id}` : `/expenses/${updated.id}`

  try {
    await apiClient.put(endpoint, updated)
    emit('saved', updated)
    emit('close')
  } catch (error) {
